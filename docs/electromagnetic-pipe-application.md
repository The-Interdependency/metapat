...
METAPAT `0.8.0` exports the canon-v4-bound pipe application and design records through the top-level `metapat` package.
...
```text
src/metapat/fixtures/three-phase-electromagnetic-pipe-v4.json
```
...
The clean-wheel smoke test verifies version `0.8.0`, the top-level constructors, the exact packaged fixture, the 18-circuit / six-system topology, catalog binding, and design digest without requiring UCNS.
