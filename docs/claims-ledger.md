...
| Boundary is the thing between things. | ROOT-STIPULATION | Boundary requires distinct multiplicity; observation does not create it. |
...
| Vector alters state; vector is inferred by scalar measurement. | DEFINITION | Vector is distinct from state metric; missing scalar observations do not establish the inference. |
...
| Structural recurrence does not transfer a domain term, mechanism, evidence standard, or conservation law. | DEFINITION / domain-qualification rule | Similarity permits comparison, not semantic transfer. |
| A domain term applies only where that domain independently licenses it. | DEFINITION / domain-qualification rule | Domain evidence owns the term; METAPAT structural resemblance is insufficient. |
...
Energy is therefore domain-qualified in v4. METAPAT does not call every state transformation energy merely because energetic systems may share some of the same abstract structure.
...
| Cross-domain comparison may recover shared structure without making domains identical. | WORKING-POSTULATE | Similarity alone is insufficient. |
| Similarity does not transfer domain-specific names, mechanisms, evidence standards, or conservation laws. | WORKING-POSTULATE | The target domain independently licenses its own terms. |
| A shared candidate may be compared after domain implementation is removed. | WORKING-POSTULATE | Removal is a falsification method, not semantic transfer. |
...
| An electrical/mechanical/linguistic/accounting implementation is identical to a METAPAT primitive. | RETRACTED_OR_SUPERSEDED | Domain implementations remain domain-specific. |

## Independent adversary standing

The double-entry bookkeeping adversary is evidence about the tested v2→v3 synthesis, not a new canon source.
...
|---|---|---|---|
| `boundary_requires_multiplicity` | INTERNAL-DERIVATION + IMPLEMENTED-CONTRACT | Encoded boundary eligibility requires at least two distinct thing instances. | External metaphysical proof. |
...
| `vector_inferred_by_scalar` | INTERNAL-DERIVATION + IMPLEMENTED-CONTRACT | Two present scalar observations differ. | Complete physical vector reconstruction or inference from missing data. |
...
| `domain_term_is_qualified` | INTERNAL-DERIVATION + IMPLEMENTED-CONTRACT | The applicable domain explicitly licenses the term; structural similarity alone is ignored as authorization. | That the domain license itself is empirically valid or that similarity establishes semantics. |
...
| `MetapatSemanticCatalog` | IMPLEMENTED-CONTRACT | Catalog v4 contains exactly 40 ordered modules and 43 exact declared derivation edges. |
...
| `tools/generate_catalog.py` | IMPLEMENTED-CONTRACT | Generates/checks `root-spine-envelope-v4.json` and `semantic-module-catalog-v4.json`. |
...
| `tools/generate_application_fixtures.py` | IMPLEMENTED-CONTRACT | Generates/checks current packaged v4 application fixtures. |
...
| root `llms.txt` | IMPLEMENTED-CONTRACT when drift check passes | Generated projection of source `LLMS` declarations; not an independent doctrine source. |
...
| “Distinction defines boundaries.” | RETRACTED_OR_SUPERSEDED | Boundary is prior to registered distinction. |
...
| Energy as primitive substrate vocabulary | RETRACTED_OR_SUPERSEDED | State/action/measurement vocabulary without universal energy naming. |
| v3 `Energy is vectors altering state through transformation across time.` as universal METAPAT definition | RETRACTED_OR_SUPERSEDED | v4 domain qualification; the target domain must license `energy`. |
| v3 “Every domain exposes some properties of Platonic energy.” | RETRACTED_OR_SUPERSEDED | Withdrawn from v4. The double-entry adversary leaves its weaker existential standing UNRESOLVED rather than falsified. |
| Catalog v2 / 52 declared edges | RETRACTED_OR_SUPERSEDED | Current catalog v4 / 43 declared edges. |
| v2/v3 root, catalog, and application fixtures | RETRACTED_OR_SUPERSEDED | v4 fixtures; all pre-v4 consumers fail closed until migrated. |
...
4. update affected catalog/application declarations, fixtures, code metadata, docs, tests, generated msdmd/llms projections, and canon manifest;
5. preserve unresolved constraints as `hmmm`.

## hmmm

The current v4 construction is deliberately incomplete in the epistemic sense: no finite domain set can certify that no later domain will add, split, refine, or falsify a candidate. The weaker historical partial-property proposition about Platonic energy remains unresolved by the double-entry adversary; it is not current v4 canon.
