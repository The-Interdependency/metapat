...
`metapat.canon_digest()` now binds two distinct surfaces without changing canon text:
...
The canon remains `metapat-canon-v1`; identity schema `2.0.0` expands coverage to the complete canon file set. A digest is identity evidence, not empirical validation or formal proof of the ontology.
...
`metapat_msdmd.ts` is generated by `tools/generate_msdmd.py` through the pinned msdmd collector. It records declaration identity, metadata digests, and graph edges while excluding vendored `.agents` declarations from the product graph.
